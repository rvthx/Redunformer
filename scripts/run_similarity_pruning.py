...
def parse_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for calibration-window sampling"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of calibration windows used to estimate head-output similarity",
    )
    parser.add_argument("--max-length", type=int, default=512, help="Calibration sequence length")
    parser.add_argument(
        "--signed-similarity",
        action="store_true",
        help="Use signed centered-cosine similarity instead of absolute similarity",
    )
    parser.add_argument(
...
def main():
    args = parse_args()
    print(
        f"Initializing similarity-guided head pruning for {args.model}: "
    ...
    similarity_dataset = get_wikitext_dataset(subset=args.dataset, split="validation")
    ...
    output_file = output_directory / (
        f"similarity_pruned_results_{model_slug}_{args.dataset}_{ratio_slug}_seed{args.seed}.json"
    )
    plan_file = output_directory / (
        f"similarity_pruning_plan_{model_slug}_{args.dataset}_{ratio_slug}_seed{args.seed}.json"
    )
    ...
    print(
        "Similarity-guided pruning evaluation completed. "
